...
上述情况属于 Update Lost 的情况，需要说明 RC 并没有禁止 Update Lost 的情况，所以有些数据库 RC 是允许这样的情况发生的。具体情况就需要看不同数据库的具体实现了。
...
| RC | 查找版本的时候，多次读操作以每次读操作的时间戳为时间戳进行查找， 不忽略 IsCommited 属性 |
| RR | 查找版本的时候，多次读操作都以事务开始的时间戳为时间戳进行查找，不忽略 IsCommited 属性 |
| S | 单纯的 MVCC 实现不了该级别，需要通过层级锁实现|

* MVCC X 锁的上锁时机视数据库具体实现而定，在事务开始的时候上锁，不会有 Update Lost，但是并发度较低。在更新的时候上锁，会发生 Update Lost，但是并发度较高，级联终止可能是一种解决 Update Lost 的方案。
